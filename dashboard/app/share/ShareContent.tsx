...
export default function ShareContent() {
  ...
  // Determine Landing Page URL based on environment
  const [landingUrl, setLandingUrl] = useState('https://readracing.vercel.app');

  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        setLandingUrl('http://localhost:5173');
      }
    }
  }, []);
  ...
  const renderRankRow = (position: number) => {
    ...
    return (
      <div key={position} className="flex items-center py-2 border-b border-[#3d1c0b]/5 last:border-0">
        ...
        <div className="flex-1 text-left font-medium flex items-center min-w-0">
          <span className="truncate text-[#3d1c0b]">{data.name}</span>
          {data.isSharer && <span className="text-[0.65rem] bg-[#e9c46a]/20 px-1.5 py-0.5 rounded ml-2 text-[#3d1c0b] shrink-0">YOU</span>}
        </div>
        <div className="font-bold whitespace-nowrap ml-2 text-[#3d1c0b]">{data.pages} <span className="font-normal text-xs text-[#5c3a2a] ml-1">pgs</span></div>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f9f5e9] p-4 font-serif text-[#3d1c0b]">
      <div className="w-full max-w-[500px] bg-[#f9f5e9] relative p-6 md:p-10 text-center shadow-none md:shadow-none">
        {/* Decorative Corners */}
        ...
        <div className="font-bold text-xl mb-8 tracking-tight animate-fade-in text-[#3d1c0b]">ReadRacing</div>
        ...
        <div className="mb-10 relative animate-slide-up delay-100">
            ...
            <h1 className="text-[2rem] leading-tight font-semibold mb-3 text-[#3d1c0b]">
                I read more than<br/>
        ...
        <div className="animate-slide-up delay-300">
            <a 
                href={landingUrl}
                className="block w-full bg-[#e9c46a] text-[#3d1c0b] py-4 rounded-lg font-semibold text-lg hover:bg-[#f0cd7d] transition-all shadow-[0_4px_12px_rgba(233,196,106,0.3)] hover:shadow-[0_6px_16px_rgba(233,196,106,0.4)] hover:-translate-y-0.5 cursor-pointer"
            >
