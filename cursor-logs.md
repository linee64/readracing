...
- Оптимизация Reader (читалки):
  ...
  - Исправлена ошибка `TypeError: Cannot read properties of undefined (reading 'content')` в `ReaderPage`:
    - Добавлены дополнительные проверки `isMounted` перед вызовами методов `epubjs`.
    - Улучшена очистка ресурсов (`destroy`) при размонтировании компонента.
    - Добавлена обработка исключений при генерации локаций и отображении книги.
...
105→  - В библиотеке карточки книг сделаны полностью кликабельными для перехода к чтению.
