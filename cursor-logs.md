...
-154→- Обновление настроек пользователя (Settings):
155→
156→## 2026-02-10
- Реструктуризация авторизации (Unified Auth).
- Интеграция EPUB книг и обновление UI Library:
  - Добавлено 16 бесплатных книг на русском языке (по 4 на жанр: Классика, Романтика, Детективы, Фэнтези) с использованием прямых ссылок на EPUB с Project Gutenberg.
  - Обновлен `useBookSearch.ts`: интегрированы метаданные (обложки, авторы, описания) для всех новых книг.
  - Модифицирован UI `SearchBar.tsx`: удалены лишние кнопки жанров, добавлена кнопка "Detectives" на английском языке для фильтрации.
  - Обновлен `Books/page.tsx`: увеличено время ожидания (timeout) прокси до 30 секунд для стабильной загрузки больших EPUB файлов.
  - Доработан `QuickViewModal.tsx`: удалена неиспользуемая вкладка "Reviews" для упрощения интерфейса.
  - Внедрена кнопка "Read book" в разделе "My Books" (`Library/page.tsx`), позволяющая открывать добавленные книги напрямую в ридере.
  - Настроена ротация прокси (allorigins, corsproxy.io, thingproxy) для обхода ограничений CORS при загрузке внешних книг. Улучшена стабильность загрузки: добавлен предварительный прямой запрос и увеличен таймаут до 45 секунд.
...
- Исправление ошибки "Failed to fetch" при авторизации:
  ...
  - В `dashboard/app/layout.tsx` добавлена логика скрытия `Sidebar` на страницах авторизации.
  - На главной странице дашборда (`dashboard/app/page.tsx`) реализована проверка сессии: если пользователь не авторизован, он перенаправляется на лендинг.
  - Ссылки на лендинге (`Header`, `Hero`, `CTA`) обновлены и теперь ведут на страницы авторизации внутри проекта дашборда.
  - Из проекта `landing` удалены старые страницы и маршруты авторизации для чистоты архитектуры.
  - Цвета `brand-beige` и `brand-black` добавлены в глобальную тему `dashboard/app/globals.css`.
- Исправление ошибок авторизации и синхронизация сессий:
  - Исправлена ошибка `AuthApiError: Invalid Refresh Token` на лендинге путем добавления обработки ошибок в `AuthContext`.
  - Синхронизированы настройки Supabase между проектами `landing` и `dashboard`:
    - Установлен общий `storageKey: 'readracing-auth-token'`. Это критически важно, так как теперь оба проекта используют одно и то же хранилище (localStorage) для сессии.
    - Теперь, если пользователь логинится в `dashboard`, лендинг мгновенно подхватывает эту сессию и меняет кнопки "Log In" на "Dashboard".
  - Включен `autoRefreshToken` и `persistSession` на лендинге для стабильной работы.
- Персонализация и безопасность данных:
  - Устранено жесткое кодирование имени "Alex". Теперь имя пользователя корректно подтягивается из Supabase Auth во всех разделах (`Dashboard`, `Library`, `Settings`, `Chat`).
  - Реализована защита данных при смене аккаунта: в `layout.tsx` добавлен механизм проверки `last_user_id`. При входе в другой аккаунт локальное хранилище книг (`IndexedDB`) автоматически очищается, чтобы новый пользователь не видел книги предыдущего.
  - Создана страница-заглушка для AI Chat (`/chat`) с современным дизайном и описанием будущих функций.
  - В настройки профиля добавлено предупреждение о том, что имя пользователя нельзя будет изменить после установки.
- Локальная разработка:
  - Временно изменены URL переходов на лендинге с `readracing-dash.vercel.app` на `http://localhost:3000` для удобства тестирования локальных изменений в дашборде.
158→  - Внедрена принудительная синхронизация (forced upsert) в компоненты [LeaderboardPreview.tsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/components/LeaderboardPreview.tsx) и [LeaderboardPage](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/app/leaderboard/page.tsx).
159→  - Теперь при каждом просмотре таблицы лидеров локальные данные пользователя из IndexedDB автоматически отправляются в Supabase.
160→  - Это гарантирует, что старые аккаунты (созданные до внедрения триггеров) получат запись в таблице `profiles`, а их прогресс (например, прочитанные 40 страниц) отобразится в глобальном рейтинге.
161→  - Синхронизация использует `upsert`, что позволяет обновлять существующие записи и создавать новые без конфликтов.
162→- Улучшение соревновательной системы:
163→  - Логика таблицы лидеров переведена на использование реальных данных из Supabase.
164→  - Реализована система замещения ботов: если в базе менее 5 реальных пользователей, таблица дополняется ботами. Как только количество реальных пользователей достигает 5 и более, боты полностью исчезают.
165→  - Данные пользователей теперь привязаны к количеству страниц (`pages_read`) для честной конкуренции.
...
- Изменение логики счетчика книг (Dashboard):
  - Обновлен расчет `booksCount` в [MetricsCards.tsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/components/MetricsCards.tsx), [LeaderboardPreview.tsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/components/LeaderboardPreview.tsx) и [leaderboard/page.tsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/app/leaderboard/page.tsx).
  - Теперь в счетчик попадают только полностью прочитанные книги (`currentPage >= totalPages`).
  - Книги, находящиеся в процессе чтения, не учитываются в общем количестве прочитанных книг, пока не будут завершены.

- Оптимизация интерфейса ридера (Dashboard):
  - В [reader/[id]/page.tsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/app/reader/[id]/page.tsx) уменьшен масштаб контейнера книги (`max-w-4xl`, `max-h-[80vh]`).
  - Добавлены внешние отступы (`p-4 md:p-8`) для предотвращения появления скроллбаров.
  - Настроен принудительный двухстраничный режим через CSS (`column-count: 2`).
  - Уменьшен размер шрифта до `16px` и оптимизированы внутренние отступы для более комфортного чтения.
  - Исправлен баг "content bleeding" (вылетание текста следующей страницы): восстановлены нативные настройки пагинации `epub.js`, удалены конфликтующие CSS-правила `overflow: hidden`, восстановлен `minSpreadWidth: 600`.

- Аналитика Vercel (Dashboard):
  - Установлен пакет `@vercel/analytics` в проект `dashboard`.
  - В [layout.tsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/dashboard/app/layout.tsx) импортирован компонент `Analytics` из `@vercel/analytics/next` (для Next.js проектов).
  - Компонент `<Analytics />` добавлен в корневой лайаут приложения.

- Аналитика Vercel (Landing):
  - Установлен пакет `@vercel/analytics` в проект `landing`.
  - В [App.jsx](file:///c:/Users/алматы2/Desktop/Aidar's%20main/ReadRacing/landing/src/App.jsx) исправлен импорт на `@vercel/analytics/react` (для React-проектов).
  - Компонент `<Analytics />` добавлен в основной макет приложения для отслеживания посещаемости.
...
149→
## 2026-02-10
- Изменения системы лидерборда:
    - Реализована синхронизация прогресса чтения с Supabase в таблицу `profiles`.
    - Лидерборд теперь загружает реальных пользователей из базы данных.
    - Введена логика: если в системе 5 и более реальных пользователей, боты полностью скрываются. Если меньше — реальные пользователи перемешиваются с ботами для заполнения таблицы.
    - Создана структура SQL для Supabase:
        - Таблица `profiles` (id, full_name, pages_read, updated_at).
        - Настроены политики RLS (просмотр для всех, редактирование только своего профиля).
        - Добавлен триггер `handle_new_user` для автоматического создания профиля при регистрации.
    - Обновлены компоненты `LeaderboardPreview.tsx` и `LeaderboardPage.tsx` для поддержки динамической загрузки и индикации загрузки (spinner).
    - Исправлен баг, при котором текущий пользователь дублировался в списке.
- Изменение системы лидерборда (ранее):
    - Переход с метрики "Книги" на "Страницы" как основную и единственную метрику соревнования.
    - Удалены переключатели метрик в `LeaderboardPreview.tsx` и `LeaderboardPage.tsx`.
...
## 2026-02-11
- ���������� �������� ��� ���������� ��������������� �������:
  - �������� `LeaderboardPreview` ���������: ������� � �������������� ���������� �������� �� ���������� ������ (flex-layout), ������� ��������� ������������ �� ��������� ����������� ��� �������.
  - �������� `CurrentBook` (Dashboard) ������� �� ����� "My Books":
    - ����� ����� ����� ������ ������������ �� 4 ��������� �������� ����.
    - ����������� ���������� ����� (grid), ����������� �������� Library, ��� ������������� ������� "��� � ������� My Books, ��� ��������������� �������".
  -  `globals.css` ��������� �������� `overflow-x: hidden` ��� `html` � `body` ��� �������������� ���������� ��������������� ������� �� ������ ��������.
  -  `ReadingPlan` � `BooksPage` (pagination) `flex` ������� �� `flex-wrap` ��� ����������� �������� ���������.
